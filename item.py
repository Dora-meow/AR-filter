...
def item_gernate(image_name='item1.png',threshold = 30,reserve=(False,False,False),reverse=False):
    # 開啟圖片
    ...
    image_name = './photo/item_' + image_name
    print(image_name)
    ...
    print(reserve)
    if reserve[0]==False:
        threshold_R= float(threshold)  # 確保 threshold 是浮點數
    else:
        threshold_R=255
    if reserve[1]==False:
    ...
    # 計算範圍的邏輯掩碼
    mask = (
        (O_array[:, :, 0] >= base[0] - threshold_R) & (O_array[:, :, 0] <= base[0] + threshold_R) &
        (O_array[:, :, 1] >= base[1] - threshold_G) & (O_array[:, :, 1] <= base[1] + threshold_G) &
        (O_array[:, :, 2] >= base[2] - threshold_B) & (O_array[:, :, 2] <= base[2] + threshold_B)
    )
